...
import (
	...
	"github.com/yomorun/cli/pkg/age"
)
...
func List() ([]ListOutput, error) {
	...
	for _, proc := range processes {
		executable := strings.ToLower(proc.Executable())
		if (executable == "yomo") || (executable == "yomo.exe") {
			pID := proc.Pid()
			if pID == myPID {
			...
			fmt.Printf("procDetails.Cmdline: %+v\n", cmdLine)

			cmdLineItems := strings.Fields(cmdLine)
			if len(cmdLineItems) <= 2 {
				continue
			}

			argumentsMap := make(map[string]string)
			for i := 2; i < len(cmdLineItems)-1; i += 2 {
				argumentsMap[cmdLineItems[i]] = cmdLineItems[i+1]
			}
			fmt.Printf("argumentsMap: %+v\n", argumentsMap)
			appID := argumentsMap["-n"]
			if appID == "" {
				appID = argumentsMap["--name"]
			}
			// appCmd := ""
			...
			listRow := ListOutput{
				...
				//
				AppID: appID,
				// Command: TruncateString(appCmd, 20),
			}
			...
			// 	listRow.Command = TruncateString(run.appCmd, 20)
			// }
